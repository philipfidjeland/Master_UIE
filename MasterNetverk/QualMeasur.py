...
def main():
    result_path = 'predictions256'
    reference_path = 'Master Netverk/LSUI/GT_test'
    ...
    for imgdir in result_dirs:
        if '.png' in imgdir:
            ...
            #reference image
            imgname = imgdir.split('predicted')[0]
            refdir = imgname+'.png'
    ...
    mpsnr = np.mean(psnr_values)
    mssim = np.mean(ssim_values)
    muiqm = np.mean(uiqm_values)
    muciqe = np.mean(uciqe_values)
